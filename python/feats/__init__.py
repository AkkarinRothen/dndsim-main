...
class AddResource(sim.feat.Feat):
    def __init__(self, name: str, uses: int, short_rest: bool = False):
        self.name = name
        self.uses = uses
    ...
    def apply(self, character):
        super().apply(character)
        if self.name in character.resources:
            character.resources[self.name].increase_max(self.uses)
        else:
            character.add_resource(self.name, self.uses, self.short_rest)
