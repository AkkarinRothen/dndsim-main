...
class DreadAmbusher(sim.feat.Feat):
    ...
    def apply(self, character: "sim.character.Character"):
        super().apply(character)
        character.add_resource('Dread Ambusher', max_uses=max(1, character.mod("wis")), short_rest=False)

    def begin_turn(self, target: "sim.target.Target"):
        self.used = False

    def attack_result(self, args):
        if args.hits() and not self.used and self.character.resources['Dread Ambusher'].use(detail="Dread Ambusher"):
            self.used = True
