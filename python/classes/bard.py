...
import sim.bardic_inspiration # Import the refactored BardicInspiration class
...
class BardicInspiration(sim.feat.Feat):
    ...
    def apply(self, character):
        ...
        character.add_resource('Bardic Inspiration', max_uses=max(1, character.mod("cha")), short_rest=True, die_size=inspiration_die(self.level))
...
class TrueStrikeAction(sim.feat.Feat):
    ...
    def action(self, target: "sim.target.Target"):
        ...
        if (
            ...
            and self.character.resources['Bardic Inspiration'].use(detail="Holy Weapon") # Using BI as a proxy for a generic bonus action resource
        ):
...
class CMEMulticlass(sim.character.Character):
    def __init__(self, level: int) -> None:
        ...
        crossbow = HandCrossbow(magic_bonus=magic_weapon)
        scimitar = Scimitar(magic_bonus=magic_weapon)
        feats.append(
