...
class Overchannel(sim.feat.Feat):
    def apply(self, character: sim.character.Character):
        super().apply(character)
        character.add_resource('Overchannel', max_uses=1, short_rest=False)
    ...
    def damage_roll(self, args):
        if (
            ...
            and self.character.resources['Overchannel'].use(detail="Overchannel")
        ):
