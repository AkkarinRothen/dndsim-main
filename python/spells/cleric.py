...
class SpiritGuardians(sim.spells.ConcentrationSpell, sim.event_loop.Listener):
    ...
    def enemy_turn(self, target: "sim.target.Target"):
        saved = False
        if target.save("wis", self.character.spells.dc()):
            saved = True
...
class TollTheDead(sim.spells.TargetedSpell):
    ...
    def cast_target(
        ...
        if not target.save("wis", character.spells.dc()):
...
class Harm(sim.spells.BasicSaveSpell):
    def __init__(self, slot: int):
        super().__init__("Harm", slot=slot, dice=14 * [6], save_ability="con", school=School.NECROMANCY)
        assert slot >= 6


class InflictWounds(sim.spells.TargetedSpell):
    def __init__(self, slot: int):
        super().__init__(
            "InflictWounds", slot, school=School.NECROMANCY
        )
    ...
    def cast_target(
        ...
        self.character.spell_attack(
            ...
            damage=DamageRoll(
                source=self.name,
                dice=(self.slot + 2) * [10], # 3d10 at level 1, +1d10 per level
            ),
        )
...
class GuardianOfFaith(sim.spells.Spell, sim.event_loop.Listener):
    ...
    def enemy_turn(self, target: "sim.target.Target"):
        if not target.save("dex", self.character.spells.dc()):
